...
import com.juridiqsystem.crm.model.dtos.DashboardFiltroRequest;
import com.juridiqsystem.crm.model.dtos.DashboardFunilEtapaResponse;
import com.juridiqsystem.crm.model.dtos.DashboardOrigemResponse;
import com.juridiqsystem.crm.model.dtos.DashboardRankingOportunidadeRow;
...
""")
     ...
     */
    @Query("""
    SELECT new com.juridiqsystem.crm.model.dtos.DashboardFunilEtapaResponse(
        e.id, e.nome, 0, COUNT(o), COALESCE(SUM(o.valor), 0), 0.0)
    FROM Etapa e
    LEFT JOIN e.oportunidades o
        ON o.situacao <> com.juridiqsystem.crm.model.enums.SituacaoOportunidade.LIXEIRA
        AND o.data_criacao BETWEEN :#{#filtro.startDate} AND :#{#filtro.endDate}
    ...
    @Query("""
    SELECT new com.juridiqsystem.crm.model.dtos.DashboardOrigemResponse(o.origem, COUNT(o), 0.0)
    FROM Oportunidade o
    WHERE o.situacao <> com.juridiqsystem.crm.model.enums.SituacaoOportunidade.LIXEIRA
      AND o.data_criacao BETWEEN :#{#filtro.startDate} AND :#{#filtro.endDate}
      AND o.etapa.funil.id IN :#{#filtro.funilIdsPermitidos}
      AND (:#{#filtro.status.size()} = 0 OR o.situacao IN :#{#filtro.status})
      AND (:#{#filtro.pipelineId} IS NULL OR o.etapa.funil.id = :#{#filtro.pipelineId})
    ...
    @Query("""
    SELECT new com.juridiqsystem.crm.model.dtos.DashboardRankingOportunidadeRow(
        u.id, u.nome,
        COUNT(CASE WHEN o.situacao = com.juridiqsystem.crm.model.enums.SituacaoOportunidade.GANHO THEN o.id END),
        COUNT(CASE WHEN o.situacao = com.juridiqsystem.crm.model.enums.SituacaoOportunidade.PERDIDO THEN o.id END),
        COALESCE(SUM(CASE WHEN o.situacao = com.juridiqsystem.crm.model.enums.SituacaoOportunidade.GANHO THEN o.valor END), 0))
    FROM Oportunidade o JOIN o.criador u
    WHERE o.situacao IN (com.juridiqsystem.crm.model.enums.SituacaoOportunidade.GANHO, com.juridiqsystem.crm.model.enums.SituacaoOportunidade.PERDIDO)
      AND o.data_criacao BETWEEN :#{#filtro.startDate} AND :#{#filtro.endDate}
      AND o.etapa.funil.id IN :#{#filtro.funilIdsPermitidos}
      AND (:#{#filtro.pipelineId} IS NULL OR o.etapa.funil.id = :#{#filtro.pipelineId})
    ...
    SELECT COUNT(o) FROM Oportunidade o
    WHERE o.etapa.id IN :etapaIds
      AND o.situacao <> com.juridiqsystem.crm.model.enums.SituacaoOportunidade.LIXEIRA
    """)
