...
public interface ProtocoloRepository extends JpaRepository<Protocolo, Long> {
    ...
    WHERE p.status = com.juridiqsystem.crm.model.enums.StatusProtocolo.ABERTO
    ...
    WHERE p.status = com.juridiqsystem.crm.model.enums.StatusProtocolo.FECHADO
      AND p.dataCriacao BETWEEN :#{#filtro.startDate} AND :#{#filtro.endDate}
      AND (:#{#filtro.userIds.size()} = 0 OR p.admin.id IN :#{#filtro.userIds})
    """)
    ...
    @Query("""
    SELECT new com.juridiqsystem.crm.model.dtos.DashboardRankingProtocoloRow(
